 ...
 * @return array
 */
function dien_tu_bk_body_classes( $classes ) {
...
add_filter( 'body_class', 'dien_tu_bk_body_classes' );
 ...
 * @return array
 */
function dien_tu_bk_post_classes( $classes ) {
	$classes[] = 'entry';

	return $classes;
}
add_filter( 'post_class', 'dien_tu_bk_post_classes', 10, 3 );
 ...
 * @return void
 */
function dien_tu_bk_pingback_header() {
	if ( is_singular() && pings_open() ) {
		echo '<link rel="pingback" href="', esc_url( get_bloginfo( 'pingback_url' ) ), '">';
	}
}
add_action( 'wp_head', 'dien_tu_bk_pingback_header' );
 ...
 * @return void
 */
function dien_tu_bk_supports_js() {
	echo '<script>document.body.classList.remove("no-js");</script>';
}
add_action( 'wp_footer', 'dien_tu_bk_supports_js' );
 ...
 * @return array
 */
function dien_tu_bk_comment_form_defaults( $defaults ) {
...
add_filter( 'comment_form_defaults', 'dien_tu_bk_comment_form_defaults' );
 ...
 * @return bool
 */
function dien_tu_bk_can_show_post_thumbnail() {
	/**
	...
	return apply_filters(
		'dien_tu_bk_can_show_post_thumbnail',
 ...
 * @return int
 */
function dien_tu_bk_get_avatar_size() {
	return 60;
}
 ...
 * @since Dien Tu BK 1.0
 */
function dien_tu_bk_continue_reading_text() {
	$continue_reading = sprintf(
		/* translators: %s: Post title. Only visible to screen readers. */
		esc_html__( 'Continue reading %s', 'dientubk' ),
 ...
 * @since Dien Tu BK 1.0
 */
function dien_tu_bk_continue_reading_link_excerpt() {
	if ( ! is_admin() ) {
		return '&hellip; <a class="more-link" href="' . esc_url( get_permalink() ) . '">' . dien_tu_bk_continue_reading_text() . '</a>';
	}
}

// Filter the excerpt more link.
add_filter( 'excerpt_more', 'dien_tu_bk_continue_reading_link_excerpt' );
 ...
 * @since Dien Tu BK 1.0
 */
function dien_tu_bk_continue_reading_link() {
	if ( ! is_admin() ) {
		return '<div class="more-link-container"><a class="more-link" href="' . esc_url( get_permalink() ) . '#more-' . esc_attr( get_the_ID() ) . '">' . dien_tu_bk_continue_reading_text() . '</a></div>';
	}
}

// Filter the content more link.
add_filter( 'the_content_more_link', 'dien_tu_bk_continue_reading_link' );

if ( ! function_exists( 'dien_tu_bk_post_title' ) ) {
	/**
	 ...
	 * @return string
	 */
	function dien_tu_bk_post_title( $title ) {
		return '' === $title ? esc_html_x( 'Untitled', 'Added to posts and pages that are missing titles', 'dientubk' ) : $title;
	}
}
add_filter( 'the_title', 'dien_tu_bk_post_title' );
 ...
 * @return string
 */
function dien_tu_bk_get_icon_svg( $group, $icon, $size = 24 ) {
	return dien_tu_bk_SVG_Icons::get_svg( $group, $icon, $size );
}
 ...
 * @return string
 */
function dien_tu_bk_change_calendar_nav_arrows( $calendar_output ) {
	$calendar_output = str_replace( '&laquo; ', is_rtl() ? dien_tu_bk_get_icon_svg( 'ui', 'arrow_right' ) : dien_tu_bk_get_icon_svg( 'ui', 'arrow_left' ), $calendar_output );
	$calendar_output = str_replace( ' &raquo;', is_rtl() ? dien_tu_bk_get_icon_svg( 'ui', 'arrow_left' ) : dien_tu_bk_get_icon_svg( 'ui', 'arrow_right' ), $calendar_output );
	return $calendar_output;
}
add_filter( 'get_calendar', 'dien_tu_bk_change_calendar_nav_arrows' );
 ...
 * @return string
 */
function dien_tu_bk_get_non_latin_css( $type = 'front-end' ) {
	...
	$font_family = apply_filters(
		'dien_tu_bk_get_localized_font_family_types',
		array(
	...
	$elements = apply_filters(
		'dien_tu_bk_get_localized_font_family_elements',
		array(
	...
	if ( ! function_exists( 'dien_tu_bk_generate_css' ) ) {
	...
	return dien_tu_bk_generate_css( // @phpstan-ignore-line.
 ...
 */
function dien_tu_bk_print_first_instance_of_block( $block_name, $content = null, $instances = 1 ) {
	$instances_count = 0;
	$blocks_content  = '';
 ...
 */
function dien_tu_bk_password_form( $output, $post = 0 ) {
	$post   = get_post( $post );
	$label  = 'pwbox-' . ( empty( $post->ID ) ? wp_rand() : $post->ID );
	$output = '<p class="post-password-message">' . esc_html__( 'This content is password protected. Please enter a password to view.', 'dientubk' ) . '</p>
	<form action="' . esc_url( site_url( 'wp-login.php?action=postpass', 'login_post' ) ) . '" class="post-password-form" method="post">
	<label class="post-password-form__label" for="' . esc_attr( $label ) . '">' . esc_html_x( 'Password', 'Post password form', 'dientubk' ) . '</label><input class="post-password-form__input" name="post_password" id="' . esc_attr( $label ) . '" type="password" size="20" /><input type="submit" class="post-password-form__submit" name="' . esc_attr_x( 'Submit', 'Post password form', 'dientubk' ) . '" value="' . esc_attr_x( 'Enter', 'Post password form', 'dientubk' ) . '" /></form>
	';
	return $output;
}
add_filter( 'the_password_form', 'dien_tu_bk_password_form', 10, 2 );
 ...
 */
function dien_tu_bk_get_attachment_image_attributes( $attr, $attachment, $size ) {
...
add_filter( 'wp_get_attachment_image_attributes', 'dien_tu_bk_get_attachment_image_attributes', 10, 3 );
