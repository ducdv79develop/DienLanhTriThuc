/* global dientubkIsDarkMode, setTimeout */

// Check the color scheme preference and inject the classes if necessary.
if ( document.body.classList.contains( 'dientubk-supports-dark-theme' ) ) {
	dientubkDarkModeEditorInit();
}
 ...
 * @return {void}
 */
function dientubkDarkModeEditorInit( attempt ) {
	...
	if ( dientubkIsDarkMode() ) {
		if ( null === container ) {
			// Try again.
			if ( attempt < maxAttempts ) {
				setTimeout(
					function() {
						dientubkDarkModeEditorInit( attempt + 1 );
					},
